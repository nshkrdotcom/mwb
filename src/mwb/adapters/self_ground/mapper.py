...
from mwb.adapters.self_ground.validators import ADAPTER_ID, DISPLAY_NAME
...
def map_run_artifacts(
    ...
    *,
    ...
    run_ref = f"run_adapter_self_ground_{slugify(source.name)}"
    ...
    return {"run_manifest": manifest, "control_metrics": metrics}
...
def _run_manifest(
    *,
    ...
    return {
        ...
        "adapter_source_kind": "self_ground_e004",
        "ingested_at": utc_now(),
        "status": status,
        "evidence_posture": "diagnostic_insufficient",
        "claim_bearing": False,
        "source_artifacts": validation.artifacts,
        "summary": {
        ...
        "metadata": {
            "adapter": {
                ...
                "notes": [
                    ...
                    "Does not define MWB core ontology",
                ],
            },
            "matrix_root": comparison.get("matrix_root"),
            "adjudication": "comparison/claim_adjudication.md",
            "legacy_readable_run_ref": f"run_self_ground_{slugify(source.name)}",
        },
    }
